    ...
    <main class="container mx-auto px-4 py-8">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex gap-2">
                <button onclick="showAddCategoryModal()" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg shadow">
                    + Tambah Kategori
                </button>
        ...
        <form method="GET" action="{{ route('pages.budgets.index') }}" class="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <select name="budget_period" id="budget_period" class="w-full md:w-1/3 px-4 py-2 border rounded-lg shadow-sm">
        ...
        <div id="addBudgetModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
            <div class="bg-white p-6 rounded-xl w-full max-w-lg shadow-lg">
                ...
                <form method="POST" action="{{ route('pages.budgets.store') }}" class="space-y-4">
                    ...
                    <input type="number" step="0.01" name="budget_amount" placeholder="Jumlah Anggaran (Rp)" class="w-full border rounded-lg px-4 py-2" required>
        ...
        <div id="addCategoryModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
            <div class="bg-white p-6 rounded-xl w-full max-w-md shadow-lg">
                ...
                <form method="POST" action="{{ route('pages.categories.store') }}" class="space-y-4">
                    @csrf
                    <input type="text" name="name" placeholder="Nama Kategori" class="w-full border rounded-lg px-4 py-2" required>
                    ...
                    <div class="flex justify-between mt-4">
                        <button type="button" onclick="hideAddCategoryModal()" class="text-gray-500 hover:underline">Batal</button>
                        <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg shadow">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
